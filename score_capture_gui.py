...
class ScoreCaptureApp:
    ...
    def create_widgets(self):
        ...
        # 새로운 항목: 해시 허밍 임계값
        bttk.Label(settings_frame, text="해시 허밍 임계값 (0-64):").grid(row=2, column=0, sticky="w", pady=5)
        self.hash_threshold_var = tk.StringVar(value="6")
        self.hash_threshold_entry = bttk.Entry(settings_frame, textvariable=self.hash_threshold_var, width=8)
        self.hash_threshold_entry.grid(row=2, column=1, sticky="e")
    ...
    def load_config(self):
        ...
        if os.path.exists(CONFIG_FILE):
            ...
            self.hash_threshold_var.set(config.get('Settings', 'hash_threshold', fallback='6'))
            
    def save_config(self):
        config = configparser.ConfigParser()
        config['Settings'] = {
            'similarity_threshold': self.similarity_var.get(),
            'hash_threshold': self.hash_threshold_var.get()
        }
        with open(CONFIG_FILE, 'w') as configfile:
            config.write(configfile)

    def is_duplicate_by_hash(self, pil_image, threshold=None):
        """PHASH 기반 중복 체크. threshold는 허밍 거리(정수)."""
        try:
            if threshold is None:
                threshold = int(self.hash_threshold_var.get())
        except Exception:
            threshold = 6
        try:
            h = imagehash.phash(pil_image)
        except Exception:
            # 안전망: 이미지가 유효하지 않으면 중복으로 보지 않음(저장 허용)
            return False
        for old_hash in self.saved_hashes:
            try:
                dist = abs(h - old_hash)
                if dist <= threshold:
                    return True
    ...
    def save_image(self, image_bgr):
        """캡처된 BGR(A) 이미지를 받아 SSIM -> 해시 중복 체크 후 저장."""
        # PIL 변환 (phash 계산을 위해)
        pil_img = Image.fromarray(cv2.cvtColor(image_bgr, cv2.COLOR_BGR2RGB))
        # SSIM 기반(기존) 판정용 그레이 변환은 capture_loop에서 이미 처리했으므로 여기선 해시 검사만 수행
        current_hash = imagehash.phash(pil_img)
        # 마지막 저장된 해시와의 간단 비교(연속 프레임 최적화용) - 유지하되, 전체 해시 리스트도 사용
        try:
            last_hash = self.last_captured_image_hash
            # 만약 마지막 해시가 있고 거의 동일하면 빠르게 리턴
            if last_hash is not None and abs(current_hash - last_hash) < 2:
                self.update_status("연속 중복(즉시) 감지. 건너뜁니다.")
                return
        except Exception:
            pass  # 안전하게 진행

        # 전체 저장 해시 목록과 비교해 중복이면 저장을 건너뜀
        if self.is_duplicate_by_hash(pil_img):
            self.update_status(f"중복 이미지(해시 기준) 감지. 저장하지 않습니다.")
            return

        # 중복이 아니면 저장 진행
        self.last_captured_image_hash = current_hash
        # BGRA -> BGR 변환(스크린샷 모드에 따라 BGRA일 수 있음)
        ...
        self.update_status(f"새 캡처 저장: {os.path.basename(filename)}")
    ...
    def capture_loop(self):
        ...
        with mss.mss() as sct:
            ...
            current_image_gray = cv2.cvtColor(current_image_bgr, cv2.COLOR_BGRA2GRAY) if current_image_bgr.shape[2] == 4 else cv2.cvtColor(current_image_bgr, cv2.COLOR_BGR2GRAY)
            if self.last_captured_image_gray is None:
                ...
                try:
                    ...
                    self.update_status("캡처 영역 크기 변경 감지. 재시도합니다.")
                    self.last_captured_image_gray = current_image_gray
